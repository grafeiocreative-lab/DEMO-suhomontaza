...
import { InsertUser, users, quotes, quoteItems, priceList, Quote, QuoteItem, PriceList } from "../drizzle/schema";
...
export async function getPriceList() {
  ...
  try {
    const result = await db.select().from(priceList).where(eq(priceList.active, 1));
    return result;
  } catch (error) {
    console.error("[Database] Failed to get price list:", error);
    throw error;
  }
}
...
export async function createOrUpdatePriceListItem(item: {
  ...
  try {
    ...
    console.error("[Database] Failed to create/update price list item:", error);
    throw error;
  }
}
...
export async function generateQuoteNumber(): Promise<string> {
  ...
  try {
    ...
    console.error("[Database] Failed to generate quote number:", error);
    throw error;
  }
}
